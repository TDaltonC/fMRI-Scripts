...
statdir = 'C:\fMRI data\DCJ\SPM\stat\Model_0008.2\FFX';
...
% Model 3:
% BOLD = evaluation_1 UP + evaluation_1 DOWN(abs) + evaluation_2 UP +
%        evaluation_2 DOWN(abs) + evaluation_3 UP + evaluation_3 DOWN(abs) +
%        evaluation_123 UP + evaluation_123 DOWN(abs) +
...
for Subj = unique(Subject)'
    ...
    for iRuns = 1:numel(Runs)
        ...
        n = 1;
        names{n} = 'evaluation;1&123';
        i = (Subject == Subj) & ...
            (strcmp(Event, 'decision_1')|strcmp(Event, 'decision_123')) & ...
            ismember(Period, Runs{iRuns});
        onsets{n} = Onset(i)+Duration(i)-(ones(numel(Onset(i)),1)*.5);
        durations{n} = .5;
        ...
        i = (Subject == Subj) & ...
            (strcmp(Event, 'decision_2')|strcmp(Event, 'decision_3')) & ...
            ismember(Period, Runs{iRuns});
        onsets{n} = Onset(i)+Duration(i)-(ones(numel(Onset(i)),1)*.5);
        durations{n} = .5;
