...
statdir = 'C:\fMRI data\DCJ\SPM\stat\Model_0002.3.1\FFX';
...
for Subj = unique(Subject)'
    ...
    for iRuns = 1:numel(Runs)
        ...
        n = 1;
        names{n} = 'evaluation;high';
        i = (Subject == Subj) & (strcmp(Event, 'evaluation_1') | ...
            strcmp(Event, 'evaluation_2') | strcmp(Event, 'evaluation_3')) & ...
            ismember(Period, Runs{iRuns});
        onsets{n} = Onset(i);
        durations{n} = 0.5;

        pmod(n).name{1} = 'EV';
        pmod(n).param{1} = Up(i)-Down(i);
        pmod(n).poly{1} = 1;
        pmod(n).name{2} = 'Risk';
        pmod(n).param{2} = Up(i)+Down(i);
        pmod(n).poly{2} = 1;
        

        n = n + 1;
        names{n} = 'evaluation;low';
