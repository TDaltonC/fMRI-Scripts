...
statdir = 'C:\fMRI data\DCJ\SPM\stat\Model_0009.2.1\FFX';
...
%Bet Increase     
        ...
        j = ((Subject == Subj) & ismember(Period, Runs{iRuns}) & ...
            (strcmp(Event, 'evaluation_2') & (Bet2-Bet1 > 1)));            
        k = ((Subject == Subj) & ismember(Period, Runs{iRuns}) & ...
            (strcmp(Event, 'evaluation_3') & (Bet3-Bet2 > 1)));
        i = (j|k);
        onsets{n} = Onset(i);
        durations{n} = .5;
        ...
        for x = 1:length(i)
            p(x) = abs((jnum(x)*(Bet2(x)-Bet1(x)) / (Bet1(x)+1)) + ...
                (knum(x) * (Bet3(x)-Bet2(x)) / (Bet2(x)+1)));
        end
 ...
 %Bet Decrease       
        ...
        % bet change for both of the two changes at once
        for x = 1:length(i)
            p(x) = (jnum(x)*Bet1(x)) + (knum(x) * Bet2(x));
        end
        pmod(n).name{1} = 'Bet;lose;param';
        pmod(n).param{1} = p(i);
        pmod(n).poly{1} = 1;
